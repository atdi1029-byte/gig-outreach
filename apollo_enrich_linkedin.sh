...
if [ "${1:-}" != "--i-know-this-is-deprecated" ]; then
    ...
    echo "To run it anyway: $0 --i-know-this-is-deprecated \"Venue Name\" VENUE_ID people.json"
    exit 2
fi
shift
...
ZB_RUN_ID="${ZB_RUN_ID:-apollo-enrich-$(date +%Y%m%dT%H%M%S)-$$}"
export ZB_RUN_ID
...
READEOF
    ...
    # --- Step 6: ZeroBounce verify through shared cost guard ---
    ZB_RESULT=$(python3 "$ZB_GUARD" verify "$EMAIL" --source "apollo+linkedin" --run-id "$ZB_RUN_ID" 2>/dev/null || true)
    ZB_STATUS=$(printf '%s' "$ZB_RESULT" | python3 -c "import json,sys; print(json.load(sys.stdin).get('status','deferred'))" 2>/dev/null || echo deferred)
    ZB_REASON=$(printf '%s' "$ZB_RESULT" | python3 -c "import json,sys; print(json.load(sys.stdin).get('reason','unknown'))" 2>/dev/null || echo guard_error)
    ZB_CHARGED=$(printf '%s' "$ZB_RESULT" | python3 -c "import json,sys; print(json.load(sys.stdin).get('charged',False))" 2>/dev/null || echo False)
    echo "  [ZB SAFE] $EMAIL → $ZB_STATUS ($ZB_REASON; charged=$ZB_CHARGED)"

    if [ "$ZB_STATUS" != "valid" ]; then
        echo "  [SKIP] Email not valid/verification deferred ($ZB_STATUS)"
        TOTAL_SKIPPED=$((TOTAL_SKIPPED + 1))
        continue
    fi
